...
export default async function ProductPage({ params }: { params: { slug: string } }) {
  const product = await db.products.bySlug(params.slug);
  if (!product) return notFound();
  const related = (await db.products.list()).filter((p) => p.categoryId === product.categoryId && p.id !== product.id).slice(0, 4);
